/**
 * AlertContext - Global alert state management (Performance Optimized)
 * Single source of truth for all alert state across the app.
 * 优化：useMemo缓存summary计算，稳定interval间隔
 */
...
export function AlertProvider({ children }: { children: ReactNode }) {
  ...
  // 优化：使用useMemo缓存summary计算
  const summary = useMemo(() => computeSummary(alerts), [alerts]);

  // Simulate real-time alerts with stable interval (不再每次随机)
  useEffect(() => {
    if (!isLive) return;
    ...
    }, 12000); // 固定12秒间隔（原来8-20秒随机，导致interval不稳定）
  ...
  const contextValue = useMemo(() => ({
    ...
    popupAlert,
    dismissPopup,
  }), [alerts, summary, isLive, acknowledgeOne, acknowledgeAll, showPopup, popupAlert, dismissPopup]);
