...
function initLiveDatasets(): LiveDatasetStats[] {
  return [
    { name: "价格时序数据集", type: "K线+Tick", coins: 0, timeRange: "--", records: 0, size: 0, status: "未采集", updateFreq: "实时", quality: 0, recordsPerSec: 0 },
    { name: "订单簿快照数据集", type: "L2/L3", coins: 0, timeRange: "--", records: 0, size: 0, status: "未采集", updateFreq: "100ms", quality: 0, recordsPerSec: 0 },
    { name: "链上交易数据集", type: "链上", coins: 0, timeRange: "--", records: 0, size: 0, status: "未采集", updateFreq: "每区块", quality: 0, recordsPerSec: 0 },
    { name: "社交情绪数据集", type: "NLP", coins: 0, timeRange: "--", records: 0, size: 0, status: "未采集", updateFreq: "5分钟", quality: 0, recordsPerSec: 0 },
    { name: "新闻事件数据集", type: "NLP", coins: 0, timeRange: "--", records: 0, size: 0, status: "未采集", updateFreq: "1分钟", quality: 0, recordsPerSec: 0 },
    { name: "DeFi协议数据集", type: "DeFi", coins: 0, timeRange: "--", records: 0, size: 0, status: "未采集", updateFreq: "每区块", quality: 0, recordsPerSec: 0 },
    { name: "基本面因子数据集", type: "基本面", coins: 0, timeRange: "--", records: 0, size: 0, status: "未采集", updateFreq: "每日", quality: 0, recordsPerSec: 0 },
    { name: "宏观经济数据集", type: "宏观", coins: 0, timeRange: "--", records: 0, size: 0, status: "未采集", updateFreq: "每日", quality: 0, recordsPerSec: 0 },
  ];
}
...
const collectionTimeline = [
  ...
  { date: "2026-02", event: "当前状态", detail: "正在采集中，暂无历史数据", milestone: true },
];
...
export default function HistoryLearning() {
  ...
  const { dataLearning, tradeRecords, aiModelPerformance, tickers, TRACKED_COINS } = trading;

  // 将真实学习事件注入到学习事件流中
  useEffect(() => {
    if (dataLearning.learningEvents.length > 0) {
      const latestEvents = dataLearning.learningEvents.slice(0, 5).map(e => ({
        id: e.id,
    ...
    setLiveCoinData(prev => {
      ...
      for (const coin of updated) {
        ...
        if (ticker) {
          ...
          coin.priceChange = ticker.priceChangePercent || coin.priceChange;
          coin.lastUpdated = "刚刚";
        }
      }
      return updated;
  ...
  const [liveQuality, setLiveQuality] = useState({
    overall: 0, completeness: 0, continuity: 0, anomalyRate: 0,
  });
  ...
  const [totalLiveSize, setTotalLiveSize] = useState(0);
  const [updatingCoins, setUpdatingCoins] = useState<Set<string>>(new Set());
  const eventIdRef = useRef(0);

  // ===== LIVE RECORDS COUNTER (1s) - 暂停，等待真实数据 =====
  // useEffect(() => {
  ...
  // ===== LIVE DATASET STATS (5s) - 暂停，等待真实数据 =====
  // useEffect(() => {
  //   const timer = setInterval(() => {
  ...
  // ===== LIVE QUALITY METRICS (8s) - 暂停，等待真实数据 =====
  // useEffect(() => {
  //   const timer = setInterval(() => {
  //     setLiveQuality(...);
  //     setDataTypeQualities(...);
  //   }, 8000);
  //   return () => clearInterval(timer);
  // }, []);
