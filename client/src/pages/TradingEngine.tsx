...
import { Zap, Wifi, Globe, Server, Activity, Route, Shield, Settings2, Pause, Layers, CandlestickChart, TrendingUp, TrendingDown, Play, X, Check, RefreshCw, Bot, Brain } from "lucide-react";
...
function LiveOrderFlow() {
  const [orders, setOrders] = useState<typeof mockRecentTrades>([]);
...
export default function TradingEngine() {
  ...
  // Sync liveExchanges when dbExchanges changes
  useEffect(() => {
    if (exchanges && exchanges.length > 0) {
      setLiveExchanges(exchanges.map((e: Exchange) => ({ ...e })));
    }
  }, [dbExchanges]);
  ...
  // 暂停假延迟和交易量自动波动，等待真实交易所数据接入
  // useEffect(() => { ... }, [liveExchanges.length]);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
        {[
          ...
          { label: "订单延迟", value: `${((sys.apiLatency || 0) * 0.07).toFixed(1)}ms`, color: (sys.apiLatency || 0) < 20 ? "blue" : "amber" },
      ...
      <Tabs defaultValue="kline" className="space-y-4">
        <TabsList className="bg-card border border-border flex-wrap">
          ...
          <TabsTrigger value="autoorder" className="data-[state=active]:bg-cyber-green/15 data-[state=active]:text-cyber-green"><Bot className="w-4 h-4 mr-1" />自动下单</TabsTrigger>
          <TabsTrigger value="fullauto" className="data-[state=active]:bg-purple-500/15 data-[state=active]:text-purple-400"><Brain className="w-4 h-4 mr-1" />全自动引擎</TabsTrigger>
        </TabsList>
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent className="space-y-3 text-xs font-mono">
                {[
                  ...
                  { label: "跨交易所延迟", value: `${Math.max(3, (sys.apiLatency || 0) - 5)}ms`, color: "text-cyber-blue" },
                  { label: "路由计算时间", value: `${((sys.apiLatency || 0) * 0.18).toFixed(1)}ms`, color: "text-cyber-blue" },
        ...
        <TabsContent value="autoorder">
          <AutoOrderPanel />
        </TabsContent>
        <TabsContent value="fullauto">
          <FullAutoPanel />
        </TabsContent>
      </Tabs>
