/**
 * Dashboard - Main command center (Enhanced v3)
...
function PriceTicker({ allCoins }: { allCoins: typeof staticCoins }) {
  ...
  return (
    <div className="relative overflow-hidden bg-card/80 border-y border-border/50 py-1.5">
      ...
      <div
        ...
        style={{ transform: `translateX(-${offset % (topCoins.length * 140)}px)` }}
      >
...
function AILearningStatus() {
  const [gen, setGen] = useState(2147);
  const [learningRate, setLearningRate] = useState(94.2);

  useEffect(() => {
    const timer = setInterval(() => {
      setGen((prev) => prev + 1);
      setLearningRate((prev) => Math.min(99.9, prev + (Math.random() - 0.3) * 0.05));
    }, 10000);
    return () => clearInterval(timer);
  }, []);

  const learningMetrics = [
    { label: "进化代数", value: `#${gen}`, icon: Dna, color: "text-cyber-blue", detail: "持续进化中" },
    { label: "模型准确率", value: `${learningRate.toFixed(1)}%`, icon: Brain, color: "text-cyber-green", detail: "+0.3% 本周" },
    { label: "学习数据量", value: "97.3亿", icon: Database, color: "text-cyber-amber", detail: "5年+全币种" },
    { label: "活跃模型", value: "8个", icon: Cpu, color: "text-cyber-blue", detail: "多模型集成" },
    { label: "知识节点", value: "248", icon: Network, color: "text-cyber-green", detail: "8大领域" },
    { label: "适应速度", value: "15min", icon: RefreshCw, color: "text-cyber-amber", detail: "新市场适应" },
  ];

  const recentLearning = [
    { time: "2分钟前", event: "BTC 4H模型权重更新完成", type: "模型更新", impact: "+0.3%" },
    { time: "8分钟前", event: "发现新链上因子: 大户30日净流入 IC=0.42", type: "因子发现", impact: "新因子" },
    { time: "15分钟前", event: "ETH网格策略参数自动优化", type: "策略优化", impact: "间距0.95%" },
    { time: "22分钟前", event: "全币种相关性矩阵更新完成", type: "知识更新", impact: "441币种" },
    { time: "35分钟前", event: "Meme币情绪检测模型精度提升", type: "模型更新", impact: "+12min预警" },
    { time: "48分钟前", event: "元学习器完成新一代进化", type: "自我进化", impact: `Gen-${gen}` },
  ];
...
export default function Dashboard() {
  ...
  const dataCoverage = useMemo(() => [
    { period: "2021", records: "12.5亿", coins: 15, accuracy: 62 },
    { period: "2022", records: "18.2亿", coins: 35, accuracy: 73 },
    { period: "2023", records: "24.8亿", coins: 60, accuracy: 85 },
    { period: "2024", records: "22.1亿", coins: 85, accuracy: 91 },
    { period: "2025", records: "19.7亿", coins: 120, accuracy: 94 },
  ], []);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Scrolling Price Ticker - live data */}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          { icon: Activity, label: "今日交易量", value: `${sys.ordersToday.toLocaleString()}笔`, change: 12.5, color: "amber" },
          { icon: Brain, label: "AI预测准确率", value: `${(94.2 + (sys.apiLatency < 15 ? 0.3 : -0.1)).toFixed(1)}%`, change: 1.8, color: "blue" },
          { icon: Globe, label: "全币种覆盖", value: `${allCoins.length}种`, change: 5.2, color: "green" },
          { icon: Shield, label: "风险评分", value: "35/100", change: -2.5, color: "green" },
        ].map((m, i) => (
      ...
      <Card className="bg-card border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-heading text-muted-foreground tracking-wider flex items-center gap-2">
            ...
            5年+历史数据学习覆盖
            <Badge variant="outline" className="text-[9px] text-cyber-blue border-cyber-blue/30 bg-cyber-blue/10 ml-2">97.3亿条记录</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-1.5">
            <AnimatePresence mode="popLayout">
              {filteredCoins.map((coin, i) => {
                ...
                return (
                  <motion.div key={coin.symbol} layout initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ delay: i * 0.01 }}
