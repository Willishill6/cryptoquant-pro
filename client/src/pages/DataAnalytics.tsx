...
const dataTimeline = [
  { year: "2021", records: "--", coins: 0, sources: 0, events: "牛市数据采集启动", status: "暂无数据" },
  { year: "2022", records: "--", coins: 0, sources: 0, events: "熊市数据+链上数据接入", status: "暂无数据" },
  { year: "2023", records: "--", coins: 0, sources: 0, events: "DeFi数据+社交数据接入", status: "暂无数据" },
  { year: "2024", records: "--", coins: 0, sources: 0, events: "Layer2+Meme币数据接入", status: "暂无数据" },
  { year: "2025", records: "--", coins: 0, sources: 0, events: "全币种覆盖+AI数据增强", status: "暂无数据" },
  { year: "2026", records: "--", coins: 0, sources: 0, events: "实时全量数据+预测数据", status: "进行中" },
];

// Historical data growth curve
const dataGrowth: { month: string; records: number; coins: number }[] = [];

export default function DataAnalytics() {
  const { coins: allCoins } = useLiveCoins();
  const sys = useSystemStatusCtx();
  const [catFilter, setCatFilter] = useState("全部");
  const throughputData: { hour: string; records: number; latency: number }[] = [];
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="relative z-10 h-full flex flex-col justify-center px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">历史数据 · 全币种采集 · AI增强分析</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
        {[
          { label: "数据源", value: "0", color: "green" },
          { label: "实时延迟", value: `${sys.apiLatency}ms`, color: sys.apiLatency < 20 ? "blue" : "amber" },
          { label: "日处理量", value: "--", color: "amber" },
          { label: "总记录数", value: "--", color: "blue" },
          { label: "历史跨度", value: "--", color: "green" },
          { label: "覆盖币种", value: `${allCoins.length}+`, color: "amber" },
          { label: "存储容量", value: "--", color: "blue" },
        ].map((stat, i) => (
      ...
      <Tabs defaultValue="history" className="space-y-4">
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                  {[
                    { type: "K线数据", period: "1m/5m/15m/1h/4h/1d", coins: "全币种", records: "--", from: "--" },
                    { type: "逐笔成交", period: "Tick级别", coins: "主流币+二线币", records: "--", from: "--" },
                    { type: "订单簿快照", period: "100ms", coins: "主流币", records: "--", from: "--" },
                    { type: "链上交易", period: "区块级别", coins: "全链", records: "--", from: "--" },
                    { type: "社交媒体", period: "实时", coins: "全币种", records: "--", from: "--" },
                    { type: "新闻/公告", period: "实时", coins: "全币种", records: "--", from: "--" },
                    { type: "DeFi TVL", period: "1h", coins: "DeFi币", records: "--", from: "--" },
                    { type: "资金费率", period: "8h", coins: "合约币种", records: "--", from: "--" },
                    { type: "清算数据", period: "实时", coins: "合约币种", records: "--", from: "--" },
                    { type: "鲸鱼地址", period: "区块级别", coins: "全链", records: "--", from: "--" },
                    { type: "矿工数据", period: "1h", coins: "PoW币", records: "--", from: "--" },
                    { type: "宏观经济", period: "1d", coins: "全局", records: "--", from: "--" },
                  ].map((dt) => (
        ...
        <TabsContent value="realtime">
          ...
          <Card className="bg-card border-border mt-4">
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                {[
                  { name: "Binance WS", type: "实时行情", status: "在线", latency: 8, records: "--" },
                  { name: "OKX WS", type: "实时行情", status: "在线", latency: 12, records: "--" },
                  { name: "Coinbase WS", type: "实时行情", status: "在线", latency: 15, records: "--" },
                  { name: "Bybit WS", type: "实时行情", status: "在线", latency: 10, records: "--" },
                  { name: "Gate.io WS", type: "实时行情", status: "在线", latency: 18, records: "--" },
                  { name: "HTX WS", type: "实时行情", status: "在线", latency: 22, records: "--" },
                  { name: "CoinGecko", type: "市场数据", status: "在线", latency: 250, records: "--" },
                  { name: "Etherscan", type: "链上数据", status: "在线", latency: 180, records: "--" },
                  { name: "Twitter/X", type: "社交数据", status: "在线", latency: 350, records: "--" },
                  { name: "DeFi Llama", type: "DeFi数据", status: "在线", latency: 200, records: "--" },
                  { name: "Glassnode", type: "链上分析", status: "在线", latency: 300, records: "--" },
                  { name: "Santiment", type: "情绪数据", status: "在线", latency: 280, records: "--" },
                  { name: "CryptoQuant", type: "链上指标", status: "在线", latency: 220, records: "--" },
                  { name: "Dune Analytics", type: "链上查询", status: "在线", latency: 500, records: "--" },
                  { name: "宏观数据API", type: "宏观经济", status: "在线", latency: 1000, records: "--" },
                ].map((source) => (
        ...
        <TabsContent value="storage">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              { name: "InfluxDB", type: "时序数据库", size: "--", records: "--", status: "未启动", usage: 0, desc: "K线/Tick数据存储" },
              { name: "TimescaleDB", type: "时序数据库", size: "--", records: "--", status: "未启动", usage: 0, desc: "订单簿/资金费率" },
              { name: "MongoDB", type: "文档数据库", size: "--", records: "--", status: "未启动", usage: 0, desc: "新闻/社交/链上数据" },
              { name: "Redis", type: "缓存", size: "--", records: "--", status: "未启动", usage: 0, desc: "实时行情/信号缓存" },
              { name: "ClickHouse", type: "分析数据库", size: "--", records: "--", status: "未启动", usage: 0, desc: "因子计算/回测数据" },
              { name: "S3 Archive", type: "冷存储", size: "--", records: "--", status: "未启动", usage: 0, desc: "历史数据归档" },
            ].map((db, i) => (
        ...
        <TabsContent value="indicators">
          <Card className="bg-card border-border">
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                {[
                  { name: "MA (移动平均)", values: ["MA5: --", "MA20: --", "MA60: --"], signal: "中性", coins: "全币种" },
                  { name: "MACD", values: ["DIF: --", "DEA: --", "MACD: --"], signal: "中性", coins: "全币种" },
                  { name: "RSI", values: ["RSI6: --", "RSI12: --", "RSI24: --"], signal: "中性", coins: "全币种" },
                  { name: "布林带", values: ["上轨: --", "中轨: --", "下轨: --"], signal: "中性", coins: "全币种" },
                  { name: "KDJ", values: ["K: --", "D: --", "J: --"], signal: "中性", coins: "全币种" },
                  { name: "OBV", values: ["OBV: --", "趋势: --", "信号: --"], signal: "中性", coins: "全币种" },
                  { name: "ATR (波动率)", values: ["ATR14: --", "ATR28: --", "波动: --"], signal: "中性", coins: "全币种" },
                  { name: "Ichimoku", values: ["转换: --", "基准: --", "云层: --"], signal: "中性", coins: "主流币" },
                ].map((indicator) => (
        ...
        <TabsContent value="reports">
          <Card className="bg-card border-border">
            ...
            <CardContent className="space-y-2">
              {[
                { name: "每日全币种交易报告", time: "--", status: "未生成", type: "日报", coins: `${allCoins.length}+币种` },
                { name: "周度策略绩效分析", time: "--", status: "未生成", type: "周报", coins: "--" },
                { name: "月度风险评估报告", time: "--", status: "未生成", type: "月报", coins: "--" },
                { name: "5年历史数据分析", time: "--", status: "未生成", type: "特别报告", coins: "--" },
                { name: "AI模型性能报告", time: "--", status: "未生成", type: "日报", coins: "--" },
                { name: "因子表现衰减报告", time: "--", status: "未生成", type: "日报", coins: "--" },
                { name: "全币种异常检测报告", time: "--", status: "未生成", type: "日报", coins: "--" },
                { name: "DeFi生态监控报告", time: "--", status: "未生成", type: "日报", coins: "--" },
              ].map((report) => (
