...
import { Settings2, Key, Bell, Shield, Palette, Database, Lock, Brain, Cpu, Plus, Trash2, RefreshCw, Eye, EyeOff, CheckCircle2, XCircle, Wifi, WifiOff, Zap, Clock, DollarSign, AlertTriangle, Copy, ExternalLink, TestTube, Activity } from "lucide-react";
...
export default function Settings() {
  // Exchange API state
  const [apis, setApis] = useState<ExchangeAPI[]>([
    { id: '1', exchange: 'Binance', apiKey: 'Bina****8f2d', secretKey: '****', permissions: ['现货交易', '合约交易', '读取账户'], status: 'connected', latency: 12, balance: 284650.32, created: '2025-08-15', lastUsed: '刚刚', ipWhitelist: '45.32.xx.xx, 103.21.xx.xx', rateLimit: 1200, rateLimitUsed: 342 },
    { id: '2', exchange: 'OKX', apiKey: 'OKX-****a3e1', secretKey: '****', passphrase: '****', permissions: ['现货交易', '读取账户'], status: 'connected', latency: 18, balance: 156230.18, created: '2025-09-20', lastUsed: '2分钟前', ipWhitelist: '45.32.xx.xx', rateLimit: 600, rateLimitUsed: 128 },
    { id: '3', exchange: 'Bybit', apiKey: 'Bybi****7c4f', secretKey: '****', permissions: ['现货交易', '合约交易', '读取账户'], status: 'connected', latency: 15, balance: 98450.55, created: '2025-10-01', lastUsed: '5分钟前', ipWhitelist: '45.32.xx.xx', rateLimit: 1000, rateLimitUsed: 215 },
    { id: '4', exchange: 'Coinbase', apiKey: 'CB-****d9b2', secretKey: '****', passphrase: '****', permissions: ['现货交易', '读取账户'], status: 'connected', latency: 45, balance: 42180.90, created: '2025-11-10', lastUsed: '15分钟前', ipWhitelist: '', rateLimit: 300, rateLimitUsed: 45 },
    { id: '5', exchange: 'Kraken', apiKey: 'Krak****e5a8', secretKey: '****', permissions: ['现货交易', '读取账户'], status: 'disconnected', latency: 0, balance: 0, created: '2025-12-05', lastUsed: '3天前', ipWhitelist: '', rateLimit: 500, rateLimitUsed: 0 },
    { id: '6', exchange: 'HTX', apiKey: 'HTX-****f1c3', secretKey: '****', permissions: ['现货交易', '读取账户'], status: 'connected', latency: 22, balance: 18920.45, created: '2026-01-15', lastUsed: '30分钟前', ipWhitelist: '45.32.xx.xx', rateLimit: 800, rateLimitUsed: 167 },
  ]);
  ...
  const handleTestConnection = useCallback((id: string) => {
    ...
    setTimeout(() => {
      ...
      setApis(prev => prev.map(a => a.id === id ? {
        ...
        balance: success ? a.balance || Math.floor(Math.random() * 200000) : 0,
      } : a));
      setTestingId(null);
      if (success) toast.success('API连接测试成功');
      else toast.error('API连接测试失败，请检查密钥');
        ...
        <TabsContent value="api">
          <div className="space-y-4">
                            ...
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div>
                                  ...
                                  <input className="w-full bg-secondary/50 border border-border rounded px-2 py-1.5 text-xs font-mono" defaultValue={api.apiKey} />
                                </div>
                                <div>
                                  <label className="text-[10px] text-muted-foreground block mb-1">Secret Key</label>
                                  <input type="password" className="w-full bg-secondary/50 border border-border rounded px-2 py-1.5 text-xs font-mono" defaultValue="••••••••••••" />
                                </div>
                                ...
                                <div>
                                  ...
                                  <input className="w-full bg-secondary/50 border border-border rounded px-2 py-1.5 text-xs font-mono" defaultValue={api.ipWhitelist} placeholder="留空则不限制" />
                                </div>
                              </div>
                              <div className="mt-3">
                                ...
                                <div className="flex gap-2 flex-wrap">
                                  {ALL_PERMISSIONS.map(perm => (
                                    ...
                                    onClick={() => {
                                      ...
                                      setApis(prev => prev.map(a => a.id === api.id ? {
                              ...
                              <div className="flex items-center gap-2 mt-3">
                                <Button size="sm" className="h-7 text-[10px] bg-cyber-blue hover:bg-cyber-blue/80 text-background" onClick={() => { setEditingId(null); toast.success('API配置已保存'); }}>保存修改</Button>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <Button variant="outline" size="sm" className="h-7 text-[10px]" onClick={() => toast.info('余额刷新中...')}>
                <DollarSign className="w-3 h-3 mr-1" />刷新余额
              </Button>
              <div className="flex-1" />
