...
export default function TradeHistory() {
  ...
  const totalFund = 10000; // 模拟账户初始资金
  ...
  const winRate = totalTrades > 0 ? (mockTrades.filter(t => t.pnl > 0).length / totalTrades * 100) : 0;
              ...
              <Card className="bg-card border-border">
                <CardContent className="p-3">
                  ...
                  <p className="font-mono text-lg font-bold text-cyber-green">
                    {mockPositions.length > 0 ? (mockPositions.filter(p => p.pnl > 0).length / mockPositions.length * 100).toFixed(0) : "0"}%
                  </p>
                </CardContent>
              </Card>
            </div>
