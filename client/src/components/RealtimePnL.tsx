/**
 * RealtimePnL - 实时盈亏追踪面板 (Performance Optimized)
 * Shows live PnL by strategy and by coin with animated updates
 * Design: Cyberpunk terminal aesthetic
 *
 * Optimizations:
 * - Removed per-item motion.div animations (8 items x every 3s = too many re-renders)
 * - Use CSS transitions instead of framer-motion for bar width
 * - Compute totalPnL inline via useMemo instead of extra useEffect + setState
...
export default function RealtimePnL() {
  ...
  // Compute totalPnL and sorted data via useMemo (no extra setState)
  const totalPnL = useMemo(() => data.reduce((s, d) => s + d.pnl, 0), [data]);
  const sortedData = useMemo(() => [...data].sort((a, b) => b.pnl - a.pnl), [data]);
  const maxAbsPnl = useMemo(() => Math.max(...data.map(d => Math.abs(d.pnl))), [data]);
          ...
          >
            {sortedData.map((entry) => (
              ...
              >
                ...
                {/* PnL bar - CSS transition instead of motion.div */}
                <div className="flex-1 h-4 relative">
                  <div className="absolute inset-0 flex items-center">
                    <div className="w-full h-1 bg-secondary/30 rounded-full relative">
                      ...
                      <div
                        className={`absolute top-0 h-full rounded-full transition-all duration-500 ${entry.pnl >= 0 ? "left-1/2" : "right-1/2"}`}
                        style={{
