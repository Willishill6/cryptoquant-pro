...
function generateBacktest(params: Record<string, number>, _strategyType: string): BacktestResult {
  ...
  const baseSharpe = 2.0 + Math.random() * 1.5;
  const sharpe = +(baseSharpe * riskFactor * aiFactor).toFixed(2);
  const annReturn = +(15 + Math.random() * 30 * sizeFactor * aiFactor).toFixed(1);
  const maxDrawdown = +(-3 - Math.random() * 15 / riskFactor).toFixed(1);
  const winRate = +(55 + Math.random() * 25 * aiFactor).toFixed(1);
  const trades = Math.round(100 + Math.random() * 500 * sizeFactor);
  const profitFactor = +(1.5 + Math.random() * 2 * aiFactor).toFixed(2);
  const calmar = +(annReturn / Math.abs(maxDrawdown)).toFixed(2);
  const totalReturn = +(annReturn * 2.5 + Math.random() * 50).toFixed(1);

  // Generate equity curve (60 data points)
  const equityCurve = Array.from({ length: 60 }, (_, i) => {
    const noise = (Math.random() - 0.45) * 2;
    const trend = annReturn / 365 * (i + 1);
    const drawdownEvent = i > 20 && i < 30 ? -Math.abs(maxDrawdown) * 0.3 * Math.sin((i - 20) / 10 * Math.PI) : 0;
    return {
      day: `D${i + 1}`,
      value: +(100000 * (1 + (trend + noise + drawdownEvent) / 100)).toFixed(0),
      benchmark: +(100000 * (1 + (trend * 0.4 + (Math.random() - 0.45) * 1.5) / 100)).toFixed(0),
    };
  });
  ...
  const monthlyReturns = Array.from({ length: 12 }, (_, i) => ({
    month: `${i + 1}月`,
    return: +((annReturn / 12) + (Math.random() - 0.4) * 8).toFixed(1),
  }));
...
export default function StrategyParamPanel({ strategy, onClose }: StrategyParamPanelProps) {
  ...
  const handleAIOptimize = useCallback(() => {
    ...
    setTimeout(() => {
      // Simulate AI finding better params
      const optimized: Record<string, number> = {};
      params.forEach(p => {
        const range = p.max - p.min;
        const jitter = (Math.random() - 0.5) * range * 0.3;
        optimized[p.key] = Math.max(p.min, Math.min(p.max, +(p.defaultValue + jitter).toFixed(2)));
        // Round to step
        optimized[p.key] = Math.round(optimized[p.key] / p.step) * p.step;
        optimized[p.key] = +optimized[p.key].toFixed(4);
      });
      setValues(optimized);
      setHasChanges(true);
      setIsRunning(false);
      toast.success("AI优化完成", { description: "已找到更优参数组合，请运行回测验证" });
